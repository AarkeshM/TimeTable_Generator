...
export default function TimetableDisplay() {
  ...
  const MOCK_DATA = {
    "1": {
        "A": {
            "columns": ["Day", "9:00-10:00", "10:00-11:00", "11:00-11:15", "11:15-12:15", "12:15-1:15", "1:15-2:00", "2:00-3:00", "3:00-4:00"],
            "data": [
                ["Mon", "Mathematics", "Physics", "Break", "Chemistry", "English", "Lunch", "Computer Sci", "-"],
                ["Tue", "Physics", "Math", "Break", "Biology", "Chemistry", "Lunch", "Sports", "Library"],
                ["Wed", "English", "Physics", "Break", "Math", "History", "Lunch", "Computer Sci", "Free"],
                ["Thu", "Chemistry", "Math", "Break", "Biology", "Physics", "Lunch", "Sports", "Library"],
                ["Fri", "Math", "Physics", "Break", "Chemistry", "English", "Lunch", "Lab", "-"]
            ]
        },
        "B": {
            "columns": ["Day", "9:00-10:00", "10:00-11:00", "11:00-11:15", "11:15-12:15"],
            "data": [
                ["Mon", "History", "English", "Break", "Math"],
                ["Tue", "Math", "History", "Break", "English"]
            ]
        }
    }
  };
  ...
  // --- INITIALIZATION ---
  useEffect(() => {
    // 1. Load the History Dropdown first
    fetchHistoryList();

    // 2. Load the actual timetable data
    if (location.state?.timetableData) {
        // If coming from the Generator Page with fresh data
        loadTimetableData(location.state.timetableData, "new");
        setIsEditing(true); // Auto-enable edit mode for fresh data
    } else {
  ...
  const fetchHistoryList = async () => {
    try {
        ...
        console.warn("Could not fetch history list (Server might be offline)");
    }
  };
  ...
  const fetchSingleTimetable = async (id = null) => {
    ...
    try {
        const token = localStorage.getItem("token");
        // If ID exists, fetch specific. If null, backend fetches latest.
        const url = id 
            ? `http://localhost:5000/api/timetable?id=${id}` 
        ...
        if (res.data.data) {
            loadTimetableData(res.data.data, res.data.id || id);
        } else {
            throw new Error("No data found");
        }
    } catch (err) {
        console.warn("API Failed or No Data. Loading MOCK DATA.");
        loadTimetableData(MOCK_DATA, "mock-version");
    } finally {
        setLoading(false);
  ...
  // --- HELPER: LOAD DATA INTO STATE ---
  const loadTimetableData = (data, id) => {
    setCurrentData(data);
    setSelectedHistoryId(id || "new");
    
    // Auto-select first available Year/Section
  ...
  const handleSave = async () => {
      ...
      try {
          ...
          if (res.data.success) {
            // 1. Refresh the history list so the new save shows in dropdown
            await fetchHistoryList(); 
            // 2. Update current ID to the new one
  ...
  // --- EXPORT: PDF WITH CUSTOM DRAWING ---
  const downloadPDF = (mode) => {
    ...
    const generateTableForSection = (year, section, startNewPage = false) => {
        ...
        autoTable(doc, {
            ...
            // Draw Custom Icon if marker found
      ...
      <header className="sticky top-0 z-40 bg-white/80 backdrop-blur-xl border-b border-slate-200/60 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
            ...
            <div className="hidden md:block relative">
                <button onClick={() => setShowHistoryMenu(!showHistoryMenu)} className="flex items-center gap-3 px-4 py-2 bg-slate-50 border border-slate-200 rounded-full text-sm font-medium hover:bg-slate-100 transition-colors">
                    ...
                    <div className="flex flex-col items-start mr-2">
                        ...
                        <span className="text-slate-700 leading-none">
                            {selectedHistoryId === 'new' ? 'Unsaved Draft' : 
                             selectedHistoryId === 'mock-version' ? 'Demo Mode' : 
                             formatDate(historyList.find(h => h._id === selectedHistoryId)?.createdAt)}
                        </span>
                    </div>
                ...
                {showHistoryMenu && (
                    <div className="absolute top-full left-1/2 -translate-x-1/2 mt-3 w-72 bg-white rounded-2xl shadow-xl border border-slate-100 p-2 z-50">
                        <div className="px-3 py-2 text-xs font-bold text-slate-400 uppercase">Saved Versions</div>
                        <div className="max-h-64 overflow-y-auto">
                            {historyList.length === 0 ? (
                                <div className="p-3 text-sm text-slate-400 italic">No saved versions found.</div>
                            ) : (
                                historyList.map((item) => (
                                    <button key={item._id} onClick={() => fetchSingleTimetable(item._id)} className="w-full text-left p-3 rounded-xl flex items-center gap-3 hover:bg-slate-50 transition-colors">
                                        <div className="w-2 h-2 rounded-full bg-indigo-500"></div>
                                        <div className="flex flex-col">
                                            <span className="text-sm font-bold text-slate-700">{formatDate(item.createdAt)}</span>
                                            <span className="text-[10px] text-slate-400">ID: {item._id.slice(-6)}</span>
                                        </div>
                                    </button>
                                ))
                            )}
                ...
                <div className="flex items-center gap-2">
                    {currentData[selectedYear] && Object.keys(currentData[selectedYear]).map(sec => (
                        <button key={sec} onClick={() => setSelectedSection(sec)}
                            className={`w-9 h-9 rounded-xl flex items-center justify-center text-sm font-bold border transition-all ${selectedSection === sec ? 'bg-indigo-600 text-white border-indigo-600 shadow-md shadow-indigo-200' : 'bg-white text-slate-600 border-slate-200 hover:border-indigo-300'}`}>
                            {sec}
                        </button>
                    ))}
                </div>
                    ...
                    <thead>
                        <tr className="border-b border-slate-100 bg-slate-50/50">
                            {schedule?.columns.map((col, i) => (
                                <th key={i} className={`py-4 px-6 text-xs font-bold uppercase text-slate-400 whitespace-nowrap ${i===0?'sticky left-0 bg-slate-50 z-10 border-r border-slate-100':''}`}>{col}</th>
                            ))}
                        </tr>
                    </thead>
                                    ...
                                    <td key={cIndex} className={`py-4 px-6 min-w-[140px] ${cIndex===0 ? 'sticky left-0 bg-white group-hover:bg-slate-50 z-10 border-r border-slate-100' : ''}`}>
                                        {cIndex === 0 ? (
                                            ...
                                            <input 
                                                ...
                                                className="w-full p-2 bg-white border border-indigo-200 rounded text-sm outline-none focus:ring-2 focus:ring-indigo-500 transition-all"
                                            />
                                        ) : (
                                            <div className="h-full w-full">
                                                {isLunchCell ? (
                                                    ...
                                                    <div className="text-sm font-medium text-slate-700" dangerouslySetInnerHTML={{__html: cell}} />
                                                )}
                                            </div>
                                        )}
                                    </td>
            ...
            <div className="flex gap-2 mb-6 overflow-x-auto no-scrollbar pb-2">
                {days.map((day, index) => (
                    <button key={index} onClick={() => setActiveMobileDay(index)}
                        className={`flex-shrink-0 px-4 py-2 rounded-full text-xs font-bold transition-all ${activeMobileDay === index ? 'bg-indigo-600 text-white shadow-md shadow-indigo-200' : 'bg-white text-slate-500 border border-slate-200'}`}>
                        {day}
                    </button>
                ))}
            </div>
                    ...
                    <div key={idx} className="relative pl-8">
                        ...
                        {isLunchCell ? (
                             <div className="flex items-center justify-center p-4 rounded-2xl bg-amber-50 border border-amber-100 border-dashed text-amber-600">
                                <Coffee size={24} />
                                <span className="ml-2 font-bold text-sm">Lunch Break</span>
                             </div>
                        ) : (
